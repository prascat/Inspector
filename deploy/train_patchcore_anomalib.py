...
def export_to_onnx_and_tensorrt(model, output_dir: Path, image_size: tuple, pattern_name: str):
    ...
    try:
        ...
        # Anomalib 모델은 InferenceBatch를 반환하므로 래퍼 필요
...
def train_patchcore(data_dir: Path, output_dir: Path, config: dict):
    ...
    try:
        ...
        # Initialize model
        print(f"\n{'='*80}")
        print(f"📊 STEP 2/4: Creating PatchCore Model")
        print(f"{'='*80}")
        print(f"   - backbone: {config['backbone']}")
        ...
        # 로깅 레벨을 INFO로 설정하여 학습 진행 상황 표시
        log_level = logging.INFO
        logging.basicConfig(
            level=log_level,
            format='%(message)s',  # 간결한 형식
            force=True
        )
        ...
        engine = Trainer(
            ...
            log_every_n_steps=1,  # 매 스텝마다 로그
        )
        
        # Train
        print(f"\n{'='*80}")
        print(f"🔥 STEP 3/4: Training PatchCore (Feature Extraction)")
        print(f"{'='*80}")
        print(f"   Backbone: {config['backbone']}")
        ...
        # Save as torch model (패턴 이름으로 저장)
        torch_path = output_dir / f"{pattern_name}.pt"
        torch.save({
