...
bool ImageProcessor::performStripInspection(const cv::Mat& roiImage, const cv::Mat& templateImage, 
    ...
    int* edgeAverageX = nullptr;
        ...
        if (!thicknesses_rear.empty()) {
                ...
                // REAR 박스 정보 반환 (ROI 절대 좌표)
                if (rearBoxCenter) *rearBoxCenter = cv::Point(boxCenterX_rear, boxCenterY_rear);
                if (rearBoxSize) *rearBoxSize = cv::Size(rearThicknessBoxWidth, rearThicknessBoxHeight);        } else {
...
bool ImageProcessor::performShapeMatching(const cv::Mat& roiImage, const cv::Mat& templateImage,
    ...
    try {
        // 입력 검증
        if (roiImage.empty() || templateImage.empty()) {
            matchScore = 0.0;
            matchLocation = cv::Point(0, 0);
            return false;
        }
        
        // ROI 이미지 복사 (필터 적용을 위해)
        cv::Mat processedRoi = roiImage.clone();
        cv::Mat processedTemplate = templateImage.clone();
        
        // ROI와 Template 크기 맞추기 (필터 전)
        // Template이 더 크면 중앙에서 ROI 크기로 추출
        if (processedTemplate.cols > processedRoi.cols || processedTemplate.rows > processedRoi.rows) {
            try {
                int offsetX = (processedTemplate.cols - processedRoi.cols) / 2;
                int offsetY = (processedTemplate.rows - processedRoi.rows) / 2;
                
                offsetX = std::max(0, std::min(offsetX, processedTemplate.cols - 1));
                offsetY = std::max(0, std::min(offsetY, processedTemplate.rows - 1));
                
                int extractW = std::min(processedRoi.cols, processedTemplate.cols - offsetX);
                int extractH = std::min(processedRoi.rows, processedTemplate.rows - offsetY);
                
                // 유효한 범위 체크
                if (extractW > 0 && extractH > 0 && 
                    offsetX >= 0 && offsetY >= 0 &&
                    offsetX < processedTemplate.cols && offsetY < processedTemplate.rows &&
                    offsetX + extractW <= processedTemplate.cols && 
                    offsetY + extractH <= processedTemplate.rows) {
                    cv::Rect extractROI(offsetX, offsetY, extractW, extractH);
                    processedTemplate = processedTemplate(extractROI).clone();
                    qDebug() << "[performShapeMatching] Template 필터 전 크기 조정: (" << offsetX << "," << offsetY << ") " 
                             << processedTemplate.cols << "x" << processedTemplate.rows;
                } else {
                    qDebug() << "[performShapeMatching] Template 추출 범위 오류, 리사이즈 사용";
                    cv::resize(processedTemplate, processedTemplate, processedRoi.size());
                }
            } catch (const cv::Exception& e) {
                qDebug() << "[performShapeMatching] Template 추출 예외:" << e.what();
                cv::resize(processedTemplate, processedTemplate, processedRoi.size());
            }
        }
        
        // 패턴의 필터 적용 (티칭 시 저장된 필터)
        if (!pattern.filters.isEmpty()) {
            for (const FilterInfo& filter : pattern.filters) {
                if (filter.enabled) {
                    applyFilter(processedRoi, processedRoi, filter);
                    applyFilter(processedTemplate, processedTemplate, filter);
                }
            }
        }
        
        // DEBUG: 중앙 추출 후 이미지 저장
        cv::imwrite("/Users/prascat/Inspector/deploy/debug_templateImage.png", processedTemplate);
        cv::imwrite("/Users/prascat/Inspector/deploy/debug_roiImage.png", processedRoi);
        
        // 그레이스케일 변환
        cv::Mat roiGray, templateGray;
        if (processedRoi.channels() == 3) {
            cv::cvtColor(processedRoi, roiGray, cv::COLOR_BGR2GRAY);
        } else {
            processedRoi.copyTo(roiGray);
        }
        
        if (processedTemplate.channels() == 3) {
            cv::cvtColor(processedTemplate, templateGray, cv::COLOR_BGR2GRAY);
        } else {
            processedTemplate.copyTo(templateGray);
        }
        
        qDebug() << "[performShapeMatching] 패턴 전체 크기 - roi:" << roiGray.cols << "x" << roiGray.rows;
        qDebug() << "[performShapeMatching] 패턴 전체 크기 - template:" << templateGray.cols << "x" << templateGray.rows;
        
        // 두 이미지 크기가 다르면 같게 조정
        if (roiGray.cols != templateGray.cols || roiGray.rows != templateGray.rows) {
            qDebug() << "[performShapeMatching] 크기 불일치! 리사이즈 시작 - template" << templateGray.cols << "x" << templateGray.rows << "-> roi" << roiGray.cols << "x" << roiGray.rows;
            cv::resize(templateGray, templateGray, roiGray.size());
            qDebug() << "[performShapeMatching] Template Gray 크기 조정 후:" << templateGray.cols << "x" << templateGray.rows;
        } else {
            qDebug() << "[performShapeMatching] 크기 일치 - 리사이즈 불필요";
        }
        
        // SHAPE 검사 박스 영역 계산
        int boxX = pattern.crimpShapeOffsetX;
        int boxY = (roiGray.rows - pattern.crimpShapeBoxHeight) / 2;
        int boxW = pattern.crimpShapeBoxWidth;
        int boxH = pattern.crimpShapeBoxHeight;
        
        qDebug() << "[DEBUG] 박스 좌표 계산:";
        qDebug() << "  roiGray.rows:" << roiGray.rows << "templateGray.rows:" << templateGray.rows;
        qDebug() << "  pattern.crimpShapeBoxHeight:" << pattern.crimpShapeBoxHeight;
        qDebug() << "  boxX:" << boxX << "boxY:" << boxY << "boxW:" << boxW << "boxH:" << boxH;
        
        // 박스 범위 검증
        if (boxX < 0) boxX = 0;
        if (boxY < 0) boxY = 0;
        if (boxX + boxW > roiGray.cols) boxW = roiGray.cols - boxX;
        if (boxY + boxH > roiGray.rows) boxH = roiGray.rows - boxY;
        
        qDebug() << "[DEBUG] 박스 범위 검증 후:";
        qDebug() << "  boxX:" << boxX << "boxY:" << boxY << "boxW:" << boxW << "boxH:" << boxH;
        ...
        // Sobel 전 박스 영역 저장
        cv::Rect shapeBox(boxX, boxY, boxW, boxH);
        cv::Mat roiBoxGray = roiGray(shapeBox).clone();
        cv::Mat templateBoxGray = templateGray(shapeBox).clone();
        
        // 박스 Gray 이미지 비교
        cv::Mat diffGray;
        cv::absdiff(roiBoxGray, templateBoxGray, diffGray);
        double minVal, maxVal, meanVal;
        cv::minMaxLoc(diffGray, &minVal, &maxVal);
        meanVal = cv::mean(diffGray)[0];
        int nonZeroCount = cv::countNonZero(diffGray);
        qDebug() << "[DEBUG] 박스 Gray 픽셀 차이 - min:" << minVal << "max:" << maxVal << "mean:" << meanVal 
                 << "nonZero:" << nonZeroCount << "/" << (roiBoxGray.cols * roiBoxGray.rows);
        
        // 디버깅: processedRoi/Template (필터 적용 후) 비교
        cv::Mat processedRoiGray, processedTemplateGray;
        if (processedRoi.channels() == 3) {
            cv::cvtColor(processedRoi, processedRoiGray, cv::COLOR_BGR2GRAY);
        } else {
            processedRoiGray = processedRoi.clone();
        }
        if (processedTemplate.channels() == 3) {
            cv::cvtColor(processedTemplate, processedTemplateGray, cv::COLOR_BGR2GRAY);
        } else {
            processedTemplateGray = processedTemplate.clone();
        }
        
        if (processedRoiGray.cols == processedTemplateGray.cols && processedRoiGray.rows == processedTemplateGray.rows) {
            cv::Mat diffProcessed;
            cv::absdiff(processedRoiGray, processedTemplateGray, diffProcessed);
            double minVal2, maxVal2, meanVal2;
            cv::minMaxLoc(diffProcessed, &minVal2, &maxVal2);
            meanVal2 = cv::mean(diffProcessed)[0];
            qDebug() << "[DEBUG] Processed (필터 후) Gray 이미지 픽셀 차이 - min:" << minVal2 << "max:" << maxVal2 << "mean:" << meanVal2;
        }
        
        // 패턴 전체에 Sobel 엣지 검출
        cv::Mat roiEdges, templateEdges;
        
        // ROI 전체 Sobel (float 연산으로 정밀도 유지)
        cv::Mat roiSobelX, roiSobelY;
        cv::Sobel(roiGray, roiSobelX, CV_32F, 1, 0, 3);
        cv::Sobel(roiGray, roiSobelY, CV_32F, 0, 1, 3);
        cv::Mat roiEdgesF;
        cv::addWeighted(roiSobelX, 0.5, roiSobelY, 0.5, 0, roiEdgesF);
        roiEdgesF.convertTo(roiEdges, CV_8U);
        
        // Template 전체 Sobel (같은 방식으로)
        cv::Mat templateSobelX, templateSobelY;
        cv::Sobel(templateGray, templateSobelX, CV_32F, 1, 0, 3);
        cv::Sobel(templateGray, templateSobelY, CV_32F, 0, 1, 3);
        cv::Mat templateEdgesF;
        cv::addWeighted(templateSobelX, 0.5, templateSobelY, 0.5, 0, templateEdgesF);
        templateEdgesF.convertTo(templateEdges, CV_8U);
        
        // 디버깅: Sobel 결과 픽셀값 비교
        if (roiEdges.cols == templateEdges.cols && roiEdges.rows == templateEdges.rows) {
            cv::Mat diffEdges;
            cv::absdiff(roiEdges, templateEdges, diffEdges);
            double minVal, maxVal, meanVal;
            cv::minMaxLoc(diffEdges, &minVal, &maxVal);
            meanVal = cv::mean(diffEdges)[0];
            int nonZeroCount = cv::countNonZero(diffEdges);
            qDebug() << "[DEBUG] Sobel 이미지 픽셀 차이 - min:" << minVal << "max:" << maxVal << "mean:" << meanVal 
                     << "nonZero:" << nonZeroCount << "/" << (roiEdges.cols * roiEdges.rows);
        }
        
        // 박스 영역만 Sobel 적용
        cv::Mat roiBoxEdges, templateBoxEdges;
        
        // 박스 영역 Sobel
        cv::Mat roiBoxSobelX, roiBoxSobelY;
        cv::Sobel(roiBoxGray, roiBoxSobelX, CV_32F, 1, 0, 3);
        cv::Sobel(roiBoxGray, roiBoxSobelY, CV_32F, 0, 1, 3);
        cv::Mat roiBoxEdgesF;
        cv::addWeighted(roiBoxSobelX, 0.5, roiBoxSobelY, 0.5, 0, roiBoxEdgesF);
        roiBoxEdgesF.convertTo(roiBoxEdges, CV_8U);
        
        // Template 박스 영역 Sobel
        cv::Mat templateBoxSobelX, templateBoxSobelY;
        cv::Sobel(templateBoxGray, templateBoxSobelX, CV_32F, 1, 0, 3);
        cv::Sobel(templateBoxGray, templateBoxSobelY, CV_32F, 0, 1, 3);
        cv::Mat templateBoxEdgesF;
        cv::addWeighted(templateBoxSobelX, 0.5, templateBoxSobelY, 0.5, 0, templateBoxEdgesF);
        templateBoxEdgesF.convertTo(templateBoxEdges, CV_8U);
        
        qDebug() << "[performShapeMatching] 박스 영역 Sobel - roi:" << roiBoxEdges.cols << "x" << roiBoxEdges.rows;
        qDebug() << "[performShapeMatching] 박스 영역 Sobel - template:" << templateBoxEdges.cols << "x" << templateBoxEdges.rows;
        qDebug() << "[performShapeMatching] 박스 좌표: x=" << boxX << " y=" << boxY << " w=" << boxW << " h=" << boxH;
        qDebug() << "[performShapeMatching] ROI 전체 크기: " << roiEdges.cols << "x" << roiEdges.rows;
        qDebug() << "[performShapeMatching] Template 전체 크기: " << templateEdges.cols << "x" << templateEdges.rows;
        
        // Template Matching 수행 (TM_CCOEFF_NORMED: 정규화된 상관계수 방식)
        cv::Mat matchResult;
        int matchMethod = cv::TM_CCOEFF_NORMED;
        
        cv::matchTemplate(roiBoxEdges, templateBoxEdges, matchResult, matchMethod);
        
        double matchMinVal, matchMaxVal;
        cv::Point minLoc, maxLoc;
        cv::minMaxLoc(matchResult, &matchMinVal, &matchMaxVal, &minLoc, &maxLoc);
        
        matchScore = matchMaxVal * 100.0;
        matchLocation = cv::Point(boxX, boxY);
        
        // 윤곽선 추출
        currentContours.clear();
        cv::findContours(roiBoxEdges.clone(), currentContours, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);
        
        templateContours.clear();
        cv::findContours(templateBoxEdges.clone(), templateContours, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);
        
        // diffMask 계산 (박스 영역만)
        diffMask = cv::Mat::zeros(roiBoxEdges.size(), CV_8UC1);
        cv::absdiff(roiBoxEdges, templateBoxEdges, diffMask);
        
        // DEBUG: diffMask 저장
        cv::imwrite("/Users/prascat/Inspector/deploy/debug_diffMask.png", diffMask);
        
        // 검사 통과 여부 반환
        return matchScore >= pattern.crimpShapeMatchRate;
