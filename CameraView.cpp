    ...
    if (event->button() == Qt::LeftButton)
    {
        ...
        if (isInspectionMode)
        {
            ...
            for (auto it = lastInspectionResult.fidResults.begin(); it != lastInspectionResult.fidResults.end(); ++it)
            {
                ...
                if (!lastInspectionResult.locations.contains(patternId))
                    continue;
            ...
            if (clickedPatternId.isNull())
            {
                for (auto it = lastInspectionResult.insResults.begin(); it != lastInspectionResult.insResults.end(); ++it)
                {
                    ...
                    if (!patternInfo || patternInfo->type != PatternType::INS)
                        continue;
...
void CameraView::showContextMenu(const QPoint &pos)
{
    ...
    else if (!currentRect.isNull())
    {
        ...
        if (!groupedRoiPatternIds.isEmpty() || !groupedFidPatternIds.isEmpty() || !groupedInsPatternIds.isEmpty())
        {
            ...
            {
                ungroupText = groupedRoiPatternIds.size() == 1 ? "ROI 그룹 해제 (1개)" : QString("ROI 그룹 해제 (%1개)").arg(groupedRoiPatternIds.size());
            }
            else if (!groupedFidPatternIds.isEmpty())
            {
                ungroupText = groupedFidPatternIds.size() == 1 ? "FID 그룹 해제 (1개)" : QString("FID 그룹 해제 (%1개)").arg(groupedFidPatternIds.size());
            }
            else
            {
                ungroupText = groupedInsPatternIds.size() == 1 ? "그룹에서 제거 (INS 1개)" : QString("그룹에서 제거 (INS %1개)").arg(groupedInsPatternIds.size());
            }
    ...
    if (targetFrameIndex >= 0 && targetFrameIndex < 4)
    {
        ...
        if (teachingWidget && targetFrameIndex < (int)teachingWidget->cameraFrames.size())
        {
            ...
            if (!frame.empty())
            {
                ...
                // ROI 유효성 검사
                if (cvRoiRect.width <= 0 || cvRoiRect.height <= 0 ||
                    cvRoiRect.x < 0 || cvRoiRect.y < 0 ||
        ...
        {
            qDebug() << QString("[CameraView] ⚠ Frame[%1] 오버레이 생성 실패: teachingWidget 조건 불충족 (tWidget=%2, size=%3)")
...
void CameraView::drawInspectionResults(QPainter &painter, const InspectionResult &result, int renderFrameIndex)
{
    drawROIPatterns(painter, result, renderFrameIndex);
    drawFIDPatterns(painter, result, renderFrameIndex);
    drawINSPatterns(painter, result, renderFrameIndex);
}

void CameraView::drawROIPatterns(QPainter &painter, const InspectionResult &result, int renderFrameIndex)
{
    // renderFrameIndex가 지정되면 그 값을, 아니면 currentFrameIndex 사용
    int targetFrameIdx = (renderFrameIndex >= 0) ? renderFrameIndex : currentFrameIndex;
...
void CameraView::drawFIDPatterns(QPainter &painter, const InspectionResult &result, int renderFrameIndex)
{
    // renderFrameIndex가 지정되면 그 값을, 아니면 currentFrameIndex 사용
    int targetFrameIdx = (renderFrameIndex >= 0) ? renderFrameIndex : currentFrameIndex;
...
void CameraView::drawINSPatterns(QPainter &painter, const InspectionResult &result, int renderFrameIndex)
{
    // renderFrameIndex가 지정되면 그 값을, 아니면 currentFrameIndex 사용
    int targetFrameIdx = (renderFrameIndex >= 0) ? renderFrameIndex : currentFrameIndex;
                                           ...
                                           const QRectF &inspRectScene, double insAngle)
{
    ...
    {
        // 기존에 계산된 좌표 재사용 (rotTopLeftVP, rotTopRightVP, rotBottomLeftVP, rotBottomRightVP)
    ...
    {
        QVector<QPoint> stripPoints;
        if (result.stripPoint1.contains(patternId))
            stripPoints.push_back(result.stripPoint1[patternId]);
        if (result.stripPoint2.contains(patternId))
            stripPoints.push_back(result.stripPoint2[patternId]);
        if (result.stripPoint3.contains(patternId))
            stripPoints.push_back(result.stripPoint3[patternId]);
        if (result.stripPoint4.contains(patternId))
            stripPoints.push_back(result.stripPoint4[patternId]);
        ...
        if (stripPoints.size() == 4)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                QPointF pointVP = sceneToViewport(painter, pointScene);
    ...
    if (result.edgeAbsolutePoints.contains(patternId))
    {
        ...
        if (patternFound && !edgePoints.isEmpty())
        {
            ...
            for (int i = 0; i < edgePoints.size(); i++)
            {
                ...
                QPointF ptVP = sceneToViewport(painter, ptScene);
                ...
                result.edgeRegressionIntercept.contains(patternId))
            {
                ...
                // Scene 좌표로 변환
                QPointF lineTopVP = sceneToViewport(painter, lineTop);
                QPointF lineBottomVP = sceneToViewport(painter, lineBottom);
                ...
                if (result.stripLengthEndPoint.contains(patternId))
                {
                    ...
                    QPointF endVP = sceneToViewport(painter, endScene);
...
void CameraView::drawINSDiffVisualization(QPainter &painter, const InspectionResult &result,
                                          ...
                                          const QRectF &inspRectScene, double insAngle)
{
    if (!result.diffMask.contains(patternId))
        return;
                                           ...
                                           const QRectF &inspRectScene, double insAngle)
{
    ...
    if (result.barrelLeftBoxRect.contains(patternId)) {
        ...
        QColor boxColor = leftPass ? QColor(0, 255, 255) : QColor(255, 165, 0);  // 시안색 또는 주황색
    ...
    if (result.barrelRightBoxRect.contains(patternId)) {
        ...
        QColor boxColor = rightPass ? QColor(255, 200, 0) : QColor(255, 165, 0);  // 밝은 주황 또는 주황색
...
void CameraView::paintEvent(QPaintEvent *event)
{
    ...
    if (isQuadViewMode)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            // 검사 결과 없으면 프레임 이미지만 보여주기
            else if (i < (int)framesCopy.size() && !framesCopy[i].empty())
            {
            ...
            // 검사 횟수 카운트 표시 (오른쪽 상단, 작은 흰색 텍스트) - 항상 표시 (0부터)
            {
                ...
                int countWidth = countFm.horizontalAdvance(countText);
                int countHeight = countFm.height();
                
                // 오른쪽 상단 위치
                QRect countRect(
                    rect.right() - countWidth - 15,
                    rect.top() + 10,
                    countWidth + 10,
                    countHeight + 4
                );
                ...
                // 반투명 배경
                painter.fillRect(countRect, QBrush(QColor(0, 0, 0, 150)));
...
QVector<QPoint> CameraView::getRotatedCorners() const
{
    ...
    if (!pattern || pattern->rect.isNull())
...
void CameraView::removePattern(const QUuid &patternId)
{
    ...
    while (!patternQueue.isEmpty())
    {
        ...
        if (patternsToDelete.contains(currentId))
            continue;
...
void CameraView::removePatternFilter(const QUuid &patternId, int filterIndex)
{
    ...
    if (!pattern)
        return;
    if (filterIndex < 0 || filterIndex >= pattern->filters.size())
        return;
...
void CameraView::movePatternFilterDown(const QUuid &patternId, int filterIndex)
{
    ...
    if (!pattern)
        return;
    if (filterIndex < 0 || filterIndex >= pattern->filters.size() - 1)
        return;
...
void CameraView::drawSelectedPatternHandles(QPainter &painter)
{
    ...
    for (const PatternInfo &pattern : patterns)
    {
        ...
        // INS STRIP 패턴의 추가 UI 요소 (그라디언트 범위 및 두께 박스)
        if (pattern.type == PatternType::INS &&
...
void CameraView::saveInspectionResultForMode(int frameIndex, const InspectionResult &result, const QPixmap &frame)
{
    if (frameIndex >= 0 && frameIndex < 4)
    {
        ...
        qDebug() << "[CameraView] 프레임" << frameIndex << "검사 결과 저장 (saveInspectionResultForMode) - 검사 횟수:" << frameInspectionCount[frameIndex];
    }
}
...
void CameraView::updateAnomalyHeatmap(const QUuid &patternId, double passThreshold)
{
    ...
    // 불량 contour 갱신
    lastInspectionResult.anomalyDefectContours[patternId] = defectContours;
